...
**New frontend dependencies:** `@xyflow/react ^12.x` (React Flow v12), `html-to-image` (PNG/SVG canvas export).
...
All `/api/canvas/*` routes require an authenticated user. Reuse pocketknife's auth middleware pattern — every request must carry a valid session/token and the resolved `userId` is used to scope all DB queries.
...
POST   /api/canvas/generate                       AI generate from prompt (streaming via fetch + ReadableStream)
...
User types prompt → clicks Generate
  → useAIGenerator.ts calls fetch() with POST /api/canvas/generate { prompt }
      (uses fetch + ReadableStream, NOT EventSource — EventSource only supports GET)
  → backend sets Content-Type: text/event-stream and streams response
  → aiGeneratorService.generate(prompt)
      ...
      → stream nodes via SSE chunks as they resolve
  → frontend reads chunks via response.body.getReader(), fades each node onto canvas as it arrives
...
- Version snapshot: every 10 saves OR on explicit "Save Version" click. `saveCount` is tracked backend-side in `diagramService` (derived from `DiagramVersion` count) so it survives page refreshes.
- Thumbnail: generated by `html-to-image` in `useCanvas.ts` on each auto-save and included in the PUT body — no separate request.
- Max versions per diagram: 20 (oldest pruned on creation of 21st)
- Restore: replaces current nodes/edges only — viewport (camera position) is intentionally not restored.
