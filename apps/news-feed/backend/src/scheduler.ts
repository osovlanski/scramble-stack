...
const preferenceAgentCron = process.env.PREFERENCE_AGENT_CRON ?? '0 3 * * 0';
const cleanupCron = '0 4 * * 0';
...
export function startScheduler(): void {
  ...
  cron.schedule(preferenceAgentCron, async () => {
    ...
    try {
      ...
      console.error('[scheduler] preference agent failed:', err);
    }
  });

  cron.schedule(cleanupCron, async () => {
    console.log('[scheduler] cleanup started');
    try {
