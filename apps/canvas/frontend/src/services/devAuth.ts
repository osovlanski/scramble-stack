 ...
 * call the backend's dev shortcut to mint a JWT for the seeded "dev@localhost"
 * user and stash it. Keeps local development zero-friction — no more pasting
 * tokens from `scripts/dev-seed.ts` into devtools.
 *
 * Disabled in production builds via `import.meta.env.DEV`, so it's impossible
 * to accidentally ship this to a real user.
 */
