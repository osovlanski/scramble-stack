 ...
 * JWT, and returns it. The frontend calls this on first load when there's no
 * auth_token in localStorage — replaces the "paste token from dev-seed.ts
 * into devtools" ritual that broke every testing session.
 *
...
function registerDevAuthRoutes(router: Router): void {
  router.post('/dev-login', async (_req: Request, res: Response) => {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.id }, secret, { expiresIn: DEV_TOKEN_TTL });
      res.json({
        success: true,
